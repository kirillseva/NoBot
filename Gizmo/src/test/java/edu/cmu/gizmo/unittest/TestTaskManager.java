...
//		managerDriver.kill();
	...
	public void testShouldRejectConflictingTaskRequestAndThenInformClientOfRejectedTask() {
		...
		// have the dummy client start a new task
		client.loadNewTask();
		
		// provide the dummy client with time to carry out the task 
		testSleep(5000);
		
		// force the robot to initiate a new task
		cobot.triggerRobotTask();
		
		// let the conflict management process play out ...
		testSleep(7000);
		
		// check to make sure the expected behavior executed
		assertTrue(cobot.gotGTR274Command());
